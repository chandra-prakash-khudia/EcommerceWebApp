...
const AdminOrders = () => {
  ...
  const getOrders = async () => {
    try {
      const { data } = await axios.get(`${window.location.origin}/api/v1/auth/all-orders` ,{withCredentials:true,});
      setOrders(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleChange = async (orderId, value) => {
    try {
      const { data } = await axios.put(`${window.location.origin}/api/v1/auth/order-status/${orderId}`, {
        status: value,
